...
- [Feature] Introduce a `IMultitypeColumnFastGet` handling in `navigable` chunk as much as possible, and keeping not-sorted into a `hash`.
...
- [Debt] Rename `Bucketor` into `Partitionor`