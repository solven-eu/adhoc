...
- [Feature] Rely on `GROUP BY ALL` syntax.
- [Feature] Requesting an `ICombination` with an `IAggregationKey` should fallback on a generic `ICombination` applying the `IAggregation`.