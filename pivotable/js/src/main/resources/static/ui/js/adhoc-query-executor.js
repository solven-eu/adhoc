...
export default {
	...
	setup(props) {
		...
		// id, generated by the server, of the latest query
		...
		function sendQuery() {
			...
			// To be checked after any `await`
			const previousSendQueryIdSnapshot = latestSentQueryId.value;
			...
			async function postFromUrl(url) {
				try {
					if (loadingV2.nbLoading >= 1) {
						if (latestQueryResultId.value) {
							console.log("Cancelling previous query", previousSendQueryIdSnapshot, " query_result_id", latestQueryResultId.value);
							userStore.authenticatedFetch(
								url +
									"?" +
									new URLSearchParams({
										query_id: latestQueryResultId.value,
									}).toString(),
								{
									method: "DELETE",
							...
							// BEWARE Cancellation of query is not functional for synchronous queries
							// It is difficult to get the id of current query with a synchronous API
							// Still, the API is relevant, as given id may be fetchable by other means (e.g. some monitoring dashboard)
						}
					}
					...
					// BEWARE We want to reset timings, not to see timings from previous query
					...
					if (synchronous) {
						...
						while (true) {
							...
							if (latestSendQueryIdSnapshot !== latestSentQueryId.value) {
								console.warn(`Received poll result for ${latestSendQueryIdSnapshot} but latest query is ${latestSentQueryId.value}`);
								return;
							}

							if (responseStateOnlyJson.retryInMs) {
								// Retry in at most 15 seconds, and after minimum 150ms
								const retryInMs = Math.max(150, Math.min(15000, responseStateOnlyJson.retryInMs));
