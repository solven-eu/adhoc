			...
			<li v-for="item in loginProviders">
				<span v-if="item.login_url">
					<span v-if="modal">
						...
						<!-- Gate on item.type (stable) rather than item.registration_id — the backend
                             sets registration_id to the profile name (e.g. "fakeUser"), not "BASIC". -->
						<span v-if="item.type == 'basic'">
							<a href="#" data-testid="login-basic-open" @click.prevent="selectedProvider = 'BASIC'"> {{ item.registration_id }} </a>
						</span>
						<span v-else>
