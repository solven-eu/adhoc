 ...
 * @property {boolean} [preparingGrid]
 * @property {boolean} [rendering]
 * @property {boolean} [sorting]
 * @property {boolean} [rowSpanning]
 *
 * @typedef TabularView
 * @property {LoadingFlags} [loading]
 */

/**
 * @param {TabularView | null | undefined} tabularView
 * @returns {boolean} true when at least one boolean flag inside `tabularView.loading` is truthy. Non-boolean
...
/**
 * Progress-bar position as a percentage (0-100). The mapping mirrors the typical lifecycle of a query round-trip —
 * sending → executing → downloading → preparingGrid → rendering — so the bar advances monotonically.
 *
 * @param {TabularView | null | undefined} tabularView
 * @returns {number} the position between 0 and 100; 100 when nothing is loading.
 */
export const loadingPercent = function (tabularView) {
	if (!isLoading(tabularView)) return 100;
	const loading = /** @type {TabularView} */ (tabularView).loading || {};
...
/**
 * Human-readable label for the current loading stage. Returns "Loaded" when idle, and a stage-specific sentence
 * otherwise. The `executing` stage has two sub-states (fetching vs. sleeping during a polling loop) reflected in
 * the message.
 *
 * @param {TabularView | null | undefined} tabularView
 * @returns {string}
