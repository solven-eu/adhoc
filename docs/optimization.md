...
`QueryStepsDagBuilder.registerRootWithDescendants` runs an `IQueryStepsDagFuser` after the DAG is built and before `getQueryDag()` returns. The interface lives in `eu.solven.adhoc.engine.dag.fuser` and is purely functional: `QueryStepsDag fuse(QueryStepsDag)`. Implementations treat the input as immutable, defensively copy `multigraph` and `inducedToInducer` only when at least one candidate is found, and return a new `QueryStepsDag` via `toBuilder()`. Implementations available:

- `NoopDagFuser` — for tests / projects that want the raw DAG.
- `PartitionorToCombinatorFuser` — rewrites a `Partitionor`-measure step as an equivalent `Combinator`-measure step when the step's groupBy already covers the Partitionor's own groupBy (so the per-partition aggregation is a no-op and the engine routes underlyings at the same granularity a Combinator would). Wired first in the default composite so the resulting Combinators participate in subgraph folding. Preserves each consumer's outgoing-edge order during rewrite — the engine uses `multigraph.outgoingEdgesOf(consumer)` order to map cuboids to `Combinator.underlyings[i]` positionally, so naively `removeVertex` + re-`addEdge` would silently swap the order and corrupt the combination's input order (we initially shipped this bug — see `TestDagCubeQuery_NullPropagation`). The order-preserving swap is centralised in `DagFuserHelpers.replaceStepMeasure`, shared with the Filtrator and Unfiltrator rewrites.
- `FiltratorToCombinatorFuser` — rewrites a `Filtrator`-measure step as a passthrough `Combinator(COALESCE)` when the Filtrator's filter is already implied by the step filter (`step.filter AND filtrator.filter == step.filter` after optimization). In that case the Filtrator would query its underlying at the same filter as the step itself, and per-cell behaves exactly like a `CoalesceCombination` on its single underlying.
- `UnfiltratorToCombinatorFuser` — rewrites an `Unfiltrator`-measure step as a passthrough `Combinator(COALESCE)` when the Unfiltrator's filter editor would leave the step filter unchanged. Concretely, `Mode.Suppress` is a no-op when none of the listed columns appear in the step filter, and `Mode.Retain` is a no-op when the step filter only constrains listed columns. The check uses `SimpleFilterEditor.suppressColumn` / `retainsColumns` on the step filter and compares the result with the original via `.equals`. The three A-to-Combinator fusers share an `AToCombinatorFuser` abstract base — subclasses provide an `isCandidate` predicate and a `buildReplacement(IMeasure) → Combinator` transform; the base owns the orchestration (candidate scan → copy-on-demand → per-step rewrite → rebuild bundle).
- `CombinatorSubgraphsFuser(minChainLength, IComposedNameStrategy)` — folds any connected subgraph of `Combinator` steps (chain or tree) into one `ComposedCombination`-backed step (registered under the short key `ComposedCombination.KEY = "COMPOSED"`, so EXPLAIN renders `Combinator[COMPOSED]`). Default `minChainLength = 2`. The fused-step name is built by a pluggable `IComposedNameStrategy` whose default preserves each internal's `name[combinationKey]` (e.g. `ratio_postcheck[DIVIDE] ∘ whole[COALESCE]`).
- `CompositeDagFuser(IQueryStepsDagFuser...)` — sequences passes as `f_n ∘ … ∘ f_1`; the default composite is `PartitionorToCombinatorFuser` then `FiltratorToCombinatorFuser` then `UnfiltratorToCombinatorFuser` then `CombinatorSubgraphsFuser`, so every per-measure rewrite into a passthrough Combinator participates in subgraph folding before the fold pass runs.
...
A node is foldable iff its measure is a `Combinator` (any number of underlyings), it has at least one incoming and at least one outgoing edge, it is not user-requested, and it is not pre-cached. The constraint that splits foldable candidates into top vs. internal is positional:

- The **top** of a fold may have multiple consumers. Each consumer is rewired to the fused step at the original positional slot of `top` in that consumer's underlyings list — one cuboid materialisation, N reads, no duplication. The `rewrite` step snapshots every consumer's outgoing-edge order before mutating and rebuilds each with `top → fusedStep` substituted, matching the pattern `DagFuserHelpers.replaceStepMeasure` uses for the A-to-Combinator fusers.
- **Internal** (non-top) nodes must have exactly one incoming edge — folding away an internal with an outside consumer would orphan that consumer. A foldable child whose `incomingEdgesOf().size() > 1` is therefore demoted to a boundary leaf even when its measure is foldable in shape.

A connected subgraph of `n >= minChainLength` internals is replaced by a single fused step whose combination evaluates the captured subgraph in one per-cell pass.

The fused step's underlyings are the *distinct* boundary leaves of the subgraph (a non-foldable, or multi-consumer, node reachable from one or more foldable internals). When the same boundary leaf is referenced from multiple foldable internals — e.g. `sum(branch_a, branch_b)` where both branches end at the same aggregator — it is registered once as an underlying, and the `ComposedCombinationPlan` routes both references through the same slot; the engine materialises that leaf's cuboid exactly once.

Linear chains are a degenerate case (every internal has exactly one underlying). Tree shapes (`Combinator.sum(chain_a, chain_b)`), shared-leaf DAGs, and subgraphs whose top has multiple consumers (e.g. `A` consumes both `B` and `E`, and `E → B → C → D`) are all handled by the same pass.

The fuser iterates candidate seeds in forward topological order (via `TopologicalOrderIterator`) so the search converges on the right top on the first seed instead of attempting too-small subgraphs first. The fused step uses the topmost node's filter / groupBy / customMarker; subgraphs with heterogeneous values on those dimensions are skipped (logged at DEBUG). The fused combination is a `ComposedCombination` resolving each constituent through the operator factory, so any combination type the project knows about composes transparently.
